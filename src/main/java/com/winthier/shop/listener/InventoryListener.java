    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (!event.isShiftClick() && isTopInventory && event.getCursor().getType() != Material.AIR) {
            if (chestShop.getChestData().getShopType() == ShopType.BUY) {
                Msg.send(player, "&cYou can't sell here.");
            }
            if (chestShop.getChestData().getShopType() == ShopType.SELL) {
                Msg.send(player, "&cUse shift click to sell items.");
            }
            return;
        }
        ...
        if (event.isShiftClick() && !isTopInventory && event.getCurrentItem().getType() != Material.AIR) {
            if (chestShop.getChestData().getShopType() == ShopType.BUY) {
                ...
                Msg.send(player, "&cYou can't put items in.");
                return;
            }
            ...
            if (chestShop.getChestData().getShopType() == ShopType.SELL) {
                ...
                if (buyItem == null) {
                    Msg.send(player, "&cYou can't sell this here.");
                    return;
                }
                int sold = 0;
            ...
            buyLoop:
                while (restStack >= buyItem.getAmount()) {
                    if (!chestShop.getChestData().isAdminShop() && !ShopPlugin.getInstance().getVaultHandler().hasMoney(chestShop.getChestData().getOwner().getUuid(), price)) {
                        Msg.send(player, "&c%s has run out of money.", chestShop.getOwnerName());
                        break buyLoop;
                    }
                    if (!chestShop.getChestData().isAdminShop() && !chestShop.addSlot(buyItem.clone())) {
                        Msg.send(player, "&cThis chest is full.");
                ...
                if (sold > 0) {
                    ...
                    Msg.send(player, "&bSold for %s.", ShopPlugin.getInstance().getVaultHandler().formatMoney(fullPrice));
                    Player ownerPlayer = chestShop.getChestData().getPlayer();
                    if (ownerPlayer != null) {
                        Msg.send(ownerPlayer, "&b%s sold %dx%s for %s to you.", player.getName(), soldItem.getAmount(), Item.getItemName(soldItem), ShopPlugin.getInstance().getVaultHandler().formatMoney(fullPrice));
                    }
                    // ShopPlugin.getInstance().logSale(chestShop, player.getName(), soldItem, fullPrice);
                }
                return;
            }
        }
        ...
        if (!event.isShiftClick() && isTopInventory && event.getCurrentItem().getType() != Material.AIR) {
            ...
            if (Double.isNaN(price)) {
                if (chestShop.getChestData().getShopType() == ShopType.BUY) {
                    Msg.send(player, "&cYou can't buy here.");
                }
                if (chestShop.getChestData().getShopType() == ShopType.SELL) {
                    Msg.send(player, "&cYou can't sell here.");
                }
                ...
                if (chestShop.getChestData().getShopType() == ShopType.BUY) {
                    Msg.send(player, "&bBuy this for %s by shift clicking.", ShopPlugin.getInstance().getVaultHandler().formatMoney(price));
                }
                if (chestShop.getChestData().getShopType() == ShopType.SELL) {
                    Msg.send(player, "&bWill pay %s for this item type.", ShopPlugin.getInstance().getVaultHandler().formatMoney(price));
                }
            }
            return;
        }
            ...
            if (chestShop.getChestData().getShopType() == ShopType.BUY) {
                ...
                if (!ShopPlugin.getInstance().getVaultHandler().takeMoney(Shopper.of(player), chestShop.getChestData().getPrice())) {
                    ...
                    if (!retours.isEmpty()) {
                        ...
                        Msg.send(player, "&cYour inventory is full.");
                        ShopPlugin.getInstance().getVaultHandler().giveMoney(Shopper.of(player), price);
                        return;
                    }
                    Msg.send(player, "&bBought for %s.", ShopPlugin.getInstance().getVaultHandler().formatMoney(price));
                    if (!chestShop.getChestData().isAdminShop()) {
                        ShopPlugin.getInstance().getVaultHandler().giveMoney(chestShop.getChestData().getOwner(), price);
                        Player ownerPlayer = chestShop.getChestData().getPlayer();
                        if (ownerPlayer != null) Msg.send(ownerPlayer, "&b%s bought %dx%s for %s from you.", player.getName(), item.getAmount(), Item.getItemName(item), ShopPlugin.getInstance().getVaultHandler().formatMoney(price));
                        event.setCurrentItem(null);
                    } else {
                        event.setCurrentItem(event.getCurrentItem());
                    }
                    // ShopPlugin.getInstance().logSale(chestShop, player.getName(), item, price);
